...
static bool copy_two_lines_of_X(uint8_t*x, long*filecount, long*fileSize, FILE *fp)
{
  ...
  while((ch_file_in != '\n') && (ch_file_in != '\r') && (y < 12))
  {
    ...
    //cursor positioning to upper serigraphy
    printf("\x1B[%d;%dH", SIZE_X * (*x) + OFFSET_X + page, SIZE_Y * y + OFFSET_X);  //3*(*x)+2+page3*(*x)+2+page, 6*y+5);
    if(y != 11)
...
static int console_get_filename(char *s, int len)
{
  ...
  *t = '\000';
  ...
  while ((c = console_getc()) != '\r')
  {
    ...
    for (int i = 0; i < sizeof(inv_filename_ch); i++)
    {
...
void file_send(void)
{
  ...
  if ((fp=fopen (fname,"r")) != NULL)
  {
      ...
      while(filecount < fileSize)
      {
        ...
        if (status > 0)
        {
          if (FD_ISSET(fd_tty, &rdfs))
          {
            ...
            rx_tty_received = read(fd_tty, input_buffer, BUFSIZ);
        ...
        if(hw_flow_ctrl)
        {
          ...
          {
            tio_error_printf("Could not get line state: %s", strerror(errno));
            break;
          }
          else
        ...
        if(x_on_state)
        {
          ...
          forward_to_tty(fd_tty, ch_file_in);

          fsync(fd_tty);                                      //update log file
    ...
    fclose(fp);
    tio_warning_printf("File send concluded!\r\n");
  } //if ((fp=fopen (fname,"r")) != NULL)
  else
    tio_warning_printf("Operation aborted: file not found!\r\n");
} //file_send(void)
