...
#include "timestamp.h"
#include "misc.h"
#include "extension.h"               //By Evandro Souza
...
#define KEY_C 0x63
#define KEY_E 0x65
#define KEY_F 0x66                  //By Evandro Souza
#define KEY_G 0x67
#define KEY_H 0x68
#define KEY_K 0x6B                  //By Evandro Souza
#define KEY_L 0x6C
...
/* By Evandro Souza - Next 6 lines without static to allow be linked with extension.c */
struct termios tio, stdin_new;
unsigned long rx_total = 0, tx_total = 0;
void (*print)(char c);
int fd_tty;
bool map_o_cr_nl = false;
bool map_o_nl_crnl = false;
static struct termios tio_old, stdout_new, stdout_old, stdin_old;
...
static bool show_parallel_keyboard;                      //By Evandro Souza
...
void tty_sync(int fd_tty)
{
    ssize_t count;

    while (tty_buffer_count > 0)
    {
        count = write(fd_tty, tty_buffer, tty_buffer_count);
        if (count < 0)
        {
            // Error
...
ssize_t tty_write(int fd_tty, const void *buffer, size_t count)
{
...
static void output_hex(char c)
{
    ...
    if (hex_char_index == 2)
    {
        ...
        ssize_t status = tty_write(fd_tty, &hex_value, 1);
        if (status < 0)
        {
...
static void toggle_line(const char *line_name, int mask, enum line_mode_t line_mode)
{
    ...
    {
        // Toggle line
        if (ioctl(fd_tty, TIOCMGET, &state) < 0)
        {
        ...
        else
        {
            ...
            if (ioctl(fd_tty, TIOCMSET, &state) < 0)
        ...
        *forward = false;
        ...
        switch (input_char)
        {
            case KEY_QUESTION:
                ...
                tio_printf(" ctrl-%c f       Send ASCII file", option.prefix_key);           //By Evandro Souza
                ...
                tio_printf(" ctrl-%c k       Show MSX keyboard activity", option.prefix_key);//By Evandro Souza
            ...
            case KEY_K:                                     //By Evandro Souza
                /* Initializes screen according with msx.keyboard file */
                if(interactive_mode && load_key_matrix())
                	show_parallel_keyboard = true;
                break;
...
{
    tcsetattr(fd_tty, TCSANOW, &tio_old);

    if (option.rs485)
    {
        /* Restore original RS-485 mode */
        rs485_mode_restore(fd_tty);
    }
...
void forward_to_tty(int fd_tty, char output_char)
{
    int status;
    ...
    if ((output_char == '\n' || output_char == '\r') && (map_o_nl_crnl))
    {
        ...
        status = tty_write(fd_tty, crlf, 2);
        if (status < 0)
        {
            tio_warning_printf("Could not write to tty device");
        }
    ...
    else
    {
        ...
        else
        {
            ...
            status = tty_write(fd_tty, &output_char, 1);
            if (status < 0)
            {
                tio_warning_printf("Could not write to tty device");
            }
...
int tty_connect(void)
{
    ...
    /* Open tty device */
    fd_tty = open(option.tty_device, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (fd_tty < 0)
    {
    ...
    /* Lock device file */
    status = flock(fd_tty, LOCK_EX | LOCK_NB);
    ...
    status = tcsetattr(fd_tty, TCSANOW, &tio);
    if (status == -1)
    {
    ...
    if (!standard_baudrate)
    {
        if (setspeed(fd_tty, option.baudrate) != 0)
        {
    ...
    show_parallel_keyboard = false;         //Evandro Souza. The default is work as standard terminal
    mount_string[0] = 0;                    //Evandro Souza.
    ...
    while (true)
    {
        ...
        {
            bool forward = false;
            if (FD_ISSET(fd_tty, &rdfs))
            {
                /* Input from tty device ready */
                ssize_t bytes_read = read(fd_tty, input_buffer, BUFSIZ);
                if (bytes_read <= 0)
                {
                ...
                for (int i=0; i<bytes_read; i++)
                {
                    ...
                    /* By Evandro Souza => Start Display pressed keys from MSX Keyboard Emulator readings */
                    if(show_parallel_keyboard)
                    {
                        /* Parse reception to show up on key maping */
                        check_input_kb_event(input_char, mount_string);
                    }
                    else //if(show_parallel_keyboard)
                    {
                        /* By Evandro Souza => End Display pressed keys from MSX Keyboard Emulator readings */
                    ...
                    }	//else if(show_parallel_keyboard)		By Evandro Souza
                }	//for (int i=0; i<bytes_read; i++)
            }
            else if (FD_ISSET(STDIN_FILENO, &rdfs))
            {
                ...
                for (int i=0; i<bytes_read; i++)
                {
                    ...
                    if (interactive_mode)
                    {
                        /* By Evandro Souza <ESC> quits MSX keyboard viewer */
                        if(show_parallel_keyboard && (input_char == '\x1B'))
                        {
                            //Returns to default terminal
                            show_parallel_keyboard = false;
                            printf("\x1B[%d;%dH\r\n", 24, 0);//last line
                            forward = false;    //Do not forward this <Esc>
                        }
                    ...
                    if (forward)
                    {
                        forward_to_tty(fd_tty, output_char);
                    }
                }
